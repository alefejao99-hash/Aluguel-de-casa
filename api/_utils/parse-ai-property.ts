...
function extractOptionalMoney(text: string, patterns: RegExp[]) {
  for (const pattern of patterns) {
    const match = text.match(pattern);
    if (!match) continue;
...
function detectIncludedCost(text: string, terms: string[]) {
  const normalized = normalizeText(text);
  return terms.some((term) =>
    normalized.includes(`${term} inclusa`) ||
    normalized.includes(`${term} incluso`) ||
    normalized.includes(`${term} incluida`) ||
    normalized.includes(`${term} incluido`) ||
    normalized.includes(`inclui ${term}`) ||
    normalized.includes(`${term} por conta do proprietario`),
  );
}
